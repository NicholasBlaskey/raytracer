...
Scenario: Intersecting a ray with a nonempty group
  Given g ← group()
    And s1 ← sphere()
    And s2 ← sphere()
    And set_transform(s2, translation(0.0, 0.0, -3.0))
    And s3 ← sphere()
    And set_transform(s3, translation(5.0, 0.0, 0.0))
    And add_child(g, s1)
    And add_child(g, s2)
    And add_child(g, s3)
  When r ← ray(point(0.0, 0.0, -5.0), vector(0.0, 0.0, 1.0))
...
Scenario: Intersecting a transformed group
  Given g ← group()
    And set_transform(g, scaling(2.0, 2.0, 2.0))
    And s ← sphere()
