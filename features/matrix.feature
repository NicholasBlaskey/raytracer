...
Scenario: A submatrix of a 4x4 matrix is a 3x3 matrix
  Given the following 4x4 matrix A:
    ...
    | -1 |  0 |  8 |  2 |
    | -7 |  1 | -1 |  1 |
  Then submatrix(A, 2, 3) is the following 3x3 matrix:
    | -6 |  1 |  1 |
    | -8 |  5 |  8 |
    | -7 |  1 | -1 |
...
Scenario: Calculating the inverse of a matrix
  ...
  Then determinant(A) = 532.0
    And cofactor(A, 2, 3) = -160.0
                 # -160/532
    And B[3,2] = -0.30075187969 
    And cofactor(A, 3, 2) = 105.0
                 # 105/532
    And B[2,3] = 0.19736842105
...
Scenario: Inverse of identity matrix
  Given A ← inverse(identity_matrix)
  Then A = identity_matrix

Scenario: Inverse of transpose verse transpose of inverse
  Given the following 4x4 matrix A:
      |  3 | -9 |  7 |  3 |
      |  3 | -8 |  2 | -9 |
      | -4 |  4 |  4 |  1 |
      | -6 |  5 | -1 |  1 |
    And inverseA ← inverse(A)
    And transposeA ← transpose(A)
  Then inverseTransposeA ← inverse(transposeA)
    And transposeInverseA ← transpose(inverseA)
    And inverseTransposeA = transposeInverseA
