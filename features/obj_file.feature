...
Scenario: Vertex records
  ...
  Then parser.vertices[1] = point(-1.0, 1.0, 0.0)
    And parser.vertices[2] = point(-1.0, 0.5, 0.0)
    And parser.vertices[3] = point(1.0, 0.0, 0.0)
    And parser.vertices[4] = point(1.0, 1.0, 0.0)
...
Scenario: Vertex normal records
  Given file ← a file containing:
    ...
    """
  When parser ← parse_obj_file(file)
  Then parser.normals[1] = vector(0.0, 0.0, 1.0)
    And parser.normals[2] = vector(0.707, 0.0, -0.707)
    And parser.normals[3] = vector(1.0, 2.0, 3.0)
...
#    And t2 = t1 # Not sure why we would need to deep equal check this...
