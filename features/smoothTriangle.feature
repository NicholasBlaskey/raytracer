...
Scenario: An intersection with a smooth triangle stores u/v
  When r ← ray(point(-0.2, 0.3, -2.0), vector(0.0, 0.0, 1.0))
    And xs ← local_intersect(tri, r)
  Then xs[0].u = 0.45
    And xs[0].v = 0.25

Scenario: A smooth triangle uses u/v to interpolate the normal
  When i ← intersection_with_uv(1.0, tri, 0.45, 0.25)
    And n ← normal_at(tri, point(0.0, 0.0, 0.0), i)
  Then n = vector(-0.5547, 0.83205, 0.0)

Scenario: Preparing the normal on a smooth triangle
  When i ← intersection_with_uv(1.0, tri, 0.45, 0.25)
    And r ← ray(point(-0.2, 0.3, -2.0), vector(0.0, 0.0, 1.0))
